...
const updateFcmToken = async (req, res, next) => {
    try {
        ...
        res.status(200).json({ success: true, message: 'FCM token registered successfully', data: { ...fcmToken, _id: fcmToken.id } });
    } catch (error) {
        next(error);
    }
};
...
const deactivateFcmToken = async (req, res, next) => {
    try {
        ...
        await prisma.fcmToken.updateMany({
            ...
            data: {
                // If isActive field exists or similar status toggles
                platform: 'inactive'
            }
        });
