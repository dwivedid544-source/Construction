...
// Placeholder — getUnreadCount not yet in chatController
router.get('/unread-count', (req, res) => res.json({ count: 0 }));
// Placeholder — getChatUsers not yet in chatController
router.get('/users', (req, res) => res.json([]));
router.post('/direct', resolveDirectChatRoomId);
router.put('/mark-read/:roomId', markMessagesAsRead);
router.get('/:roomId', getRoomMessages);
router.post('/', sendMessage);
// Placeholder — updateMessageAttachments not yet in chatController
router.patch('/:messageId/attachments', (req, res) => res.status(501).json({ message: 'Not implemented' }));
