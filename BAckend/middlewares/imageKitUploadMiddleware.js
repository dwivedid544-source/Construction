...
let imagekit = null;
const imagekitId = process.env.IMAGEKIT_ID || (process.env.IMAGEKIT_URL_ENDPOINT ? process.env.IMAGEKIT_URL_ENDPOINT.split('/').pop() : '');
const apiKey = process.env.IMAGEKIT_PUBLIC_KEY || process.env.IMAGEKIT_API_KEY;
const apiSecret = process.env.IMAGEKIT_PRIVATE_KEY || process.env.IMAGEKIT_API_SECRET;
const urlEndpoint = process.env.IMAGEKIT_URL_ENDPOINT || (imagekitId ? `https://ik.imagekit.io/${imagekitId}` : '');

if (apiKey && apiSecret && (imagekitId || urlEndpoint)) {
    try {
...
const imageKitUpload = async (req, res, next) => {
    ...
    if (files.length === 0 || !imagekit) return next();
